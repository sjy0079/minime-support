...
router.get('/', multipartMiddleware, async function (req, res) {
  ...
  const isFc = req.query.isFc;
  const isAj = req.query.isAj;
...
function modifyScore(res, db, profileId, mid, score, levelStr, isFcB, isAjB) {
  try {
    const result = db.prepare(
      'SELECT * FROM c3_user_music WHERE ' +
      `profile_id = '${profileId}' AND music_id = '${mid}' AND level = '${levelStr}'`
    ).all();
    // 插入数据
    if (result.length === 0) {
    ...
    // 修改数据
      ...
      const updateStmt = db.prepare(
        'UPDATE c3_user_music SET score = ? AND is_full_combo = ? AND is_all_justice = ? WHERE profile_id = ? AND music_id = ? AND level = ?');
